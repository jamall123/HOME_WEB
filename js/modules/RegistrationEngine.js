 ...
 * Handles the complete registration workflow for Academy Courses (Free & Paid).
 * Validates inputs, handles duplicate checking, dynamic bank loading, file uploads, and state management.
 */
...
window.RegistrationEngine = {
    ...
    attachBottomSheetGestures() {
        ...
        handle.addEventListener('touchstart', (e) => {
            // Prevent dragging if currently loading/submitting
    ...
    openRegistration(courseId, courseTitle, isPaid) {
    ...
    validateForm() {
        ...
        // 2. Email: Optional, but if provided must be valid format
        if (emailVal) {
        ...
        // 4. Phone: Must contain at least 9 digits
    ...
    async loadBankAccounts() {
        ...
        try {
            ...
            this.loadedBankAccounts = [];
            snap.forEach(doc => {
                this.loadedBankAccounts.push({ id: doc.id, ...doc.data() });
            });
    ...
    async uploadReceipt(file) {
        ...
        const fileName = `receipts/${this.currentCourseId}/${Date.now()}_${Math.floor(Math.random()*1000)}.${ext}`;
