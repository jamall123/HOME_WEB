...
class MediaManagerClass {
    ...
    async handleFiles(files) {
        ...
        for (let i = 0; i < processedFiles.length; i++) {
            ...
            const uploadTask = this.storage.ref(filePath).put(file);
            ...
            uploadTask.on('state_changed', 
                ...
                async () => {
                    ...
                    await (await import('../core/CommandBus.js')).commandBus.dispatch({ domain: 'generic', action: 'add', payload: { collection: 'media_library', data: {
                        name: file.name,
                        path: filePath,
                        url: url,
                        type: file.type,
    ...
    async loadGallery() {
        ...
        try {
            ...
            let query = this.db.collection('media_library')
                .where('status', '==', this.currentView)
                ...
                html += `
                    <div style="border: 1px solid var(--border-color); padding: 0.5rem; border-radius: 8px; text-align: center; background: var(--bg-surface); position: relative; overflow: hidden; transition: transform 0.2s; opacity: ${isTrash ? '0.7' : '1'};">
                        ...
                        <div style="display: flex; gap: 0.5rem; justify-content: center; position: absolute; bottom: 0; left: 0; right: 0; background: rgba(0,0,0,0.8); padding: 0.5rem; transform: translateY(100%); transition: transform 0.2s;" class="media-actions">
                            ${isTrash ? `
                                <button onclick="restoreMedia('${doc.id}')" class="btn btn-secondary" style="padding: 0.2rem 0.5rem; font-size: 0.8rem; color: #00d2d3;" title="استعادة"><i class="fas fa-undo"></i></button>
                                <button onclick="deleteMedia('${doc.id}', '${data.path}')" class="btn btn-secondary" style="padding: 0.2rem 0.5rem; font-size: 0.8rem; color: #ff4757;" title="حذف نهائي"><i class="fas fa-trash-alt"></i></button>
                            ` : `
                                ...
                                <button onclick="deleteMedia('${doc.id}', '${data.path}')" class="btn btn-secondary" style="padding: 0.2rem 0.5rem; font-size: 0.8rem; color: #ff4757;" title="نقل للمهملات"><i class="fas fa-trash"></i></button>
                            `}
                        </div>
                    </div>
                `;
    ...
    async deleteMedia(docId, path) {
        if (this.currentView === 'active') {
            ...
            const doc = await this.db.collection('media_library').doc(docId).get();
            if (doc.exists && doc.data().usageCount > 0) {
                if(!confirm(`هذا الملف مُستخدم في ${doc.data().usageCount} مكان! نقل إلى المهملات؟`)) return;
            } else {
                if(!confirm('نقل هذا الملف إلى سلة المحذوفات؟')) return;
            }

            // Soft delete — set deletedAt flag, mediaController trigger ignores this
            await this.db.collection('media_library').doc(docId).update({
                deleted: true,
            ...
            try {
                ...
                await this.db.collection('media_library').doc(docId).delete();
