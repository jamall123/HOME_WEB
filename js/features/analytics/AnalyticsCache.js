...
export class AnalyticsCacheClass {
    ...
    async get(collection, docId) {
        ...
        // 3. Repository fetch (Network)
        try {
            if (!navigator.onLine) return null;

            let data = null;
            if (collection === 'studentProgress') {
                data = await StudentProgressRepository.getStudentProgress(docId);
            } else if (collection === 'courseAnalytics') {
                data = await StudentProgressRepository.getCourseAnalytics(docId);
            }
            ...
            console.error('[AnalyticsCache] Repository fetch failed', e);
            return null;
        }
    }
