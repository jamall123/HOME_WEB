...
export class CertificateService {
    static async generateAndStore(courseId, userId) {
        ...
        const courseTitle = course ? course.title : 'دورة تدريبية';
        
        const user = await UserRepository.getUser(userId);
        let studentName = user ? user.fullName : 'طالب';
        ...
        if (!user) {
           ...
           if (cachedUser) {
               try {
                   studentName = JSON.parse(cachedUser).fullName || studentName;
               } catch (e) {
                   console.warn("Could not parse cached student data");
               }
           }
        }
        ...
        await CertificateRepository.createCertificateWithId(certId, {
            courseId,
            userId,
            studentName,
            courseTitle,
    ...
    static async showCertificateModal(certId, studentName, courseTitle, dateStr, verifyUrl) {
        ...
        // Generate QR Code
        setTimeout(() => {
            new window.QRCode(document.getElementById('cert-qrcode'), {
                text: verifyUrl,
                width: 100,
                height: 100
            });
