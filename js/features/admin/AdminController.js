...
class AdminControllerClass {
    ...
    async loadAuditLog(isLoadMore = false) {
        ...
        try {
            // Use AdminRepository for pagination (stubbed limit here, full pagination is in Repo or can just load 30)
            const logs = await AdminRepository.getAuditLogs(30);

            if (!isLoadMore) this.loadedData.auditlog = [];
            this.loadedData.auditlog.push(...logs);
            this.cursors.auditlog = logs.length > 0 ? logs[logs.length - 1] : null;
            ...
            if (this.loadedData.auditlog.length === 0) {
                ...
                tbody.innerHTML = this.loadedData.auditlog.map(log => {
                    ...
                    return `<tr>
                        <td>${log.performedBy || log.userId || log.admin || 'النظام'}</td>
                        ...
                        <td style="max-width:300px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="${log.description || log.details || ''}"><code style="font-size:0.78rem; color:var(--text-secondary);">${log.collection || ''}</code> ${log.description || log.details || log.targetId || '—'}</td>
