...
class GlobalControllerClass {
    ...
    async loadDynamicData() {
        ...
        try {
            ...
            const pageDoc = await ContentRepository.getPageContent(pageKey);
    ...
    async loadLiveStats() {
        ...
        try {
            const [coursesSnap, postsSnap, storiesSnap] = await Promise.all([
                CourseRepository.getCoursesCount(),
                ContentRepository.getPublishedPostsCount(),
                ContentRepository.getPublishedStoriesCount()
            ]);

            const coursesCounter = document.getElementById('stat-courses');
            if (coursesCounter && coursesSnap) coursesCounter.setAttribute('data-target', coursesSnap.data().count);

            const postsCounter = document.getElementById('stat-posts');
            if (postsCounter && postsSnap) postsCounter.setAttribute('data-target', postsSnap.data().count);

            const storiesCounter = document.getElementById('stat-stories');
            if (storiesCounter && storiesSnap) storiesCounter.setAttribute('data-target', storiesSnap.data().count);
