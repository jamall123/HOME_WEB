import"./ThemeManager-Df5bj4E3.js";import{t as a}from"./GlobalController-Df_3lCaU.js";/* empty css             */var w=class{constructor(){this.db=window.firebase?window.firebase.firestore():null}async getStories(e="all",i=50){if(!this.db)return a.error("StoriesService: Firebase not initialized."),[];try{let t=this.db.collection("successStories").where("isPublished","==",!0);return e!=="all"&&(t=t.where("category","==",e)),t=t.orderBy("createdAt","desc").limit(i),(await t.get()).docs.map(s=>({id:s.id,...s.data()}))}catch(t){throw a.error("StoriesService: Error fetching stories",t),t}}async getStoryById(e){if(!this.db)return null;try{const i=await this.db.collection("successStories").doc(e).get();return i.exists?{id:i.id,...i.data()}:null}catch(i){throw a.error("StoriesService: Error fetching story",i),i}}},l=new w,y=class{constructor(){this.grid=document.getElementById("storiesGrid")}escapeHtml(e){if(!e)return"";const i=document.createElement("div");return i.textContent=e,i.innerHTML}renderLoading(){this.grid&&(this.grid.innerHTML='<div class="loading"><i class="fas fa-spinner fa-spin"></i> جاري التحميل...</div>')}renderEmpty(){this.grid&&(this.grid.innerHTML='<div class="empty-state"><i class="fas fa-inbox"></i><p>لا توجد قصص بعد في هذا التصنيف.</p></div>')}renderError(){this.grid&&(this.grid.innerHTML='<div class="error-state">حدث خطأ في تحميل القصص.</div>')}renderStories(e){if(!this.grid)return;const i=document.createDocumentFragment();e.forEach(t=>{const s=(t.personName||"؟").split(" ").slice(0,2).map(b=>b[0]).join(""),n=t.publishedAt?new Date(t.publishedAt.seconds*1e3).toLocaleDateString("ar-SD",{year:"numeric",month:"long"}):"",m=(t.story||"").replace(/<[^>]*>/g,"").slice(0,180)+"…",c=document.createElement("article");c.className="story-card glass-card";const o=t.coverImage||t.image||t.cover||t.personAvatar,h=o?`<img src="${this.escapeHtml(o)}" alt="${this.escapeHtml(t.personName||"")}" loading="lazy" decoding="async" style="position:absolute; inset:0; width:100%; height:100%; object-fit:cover;" onerror="this.style.display='none';">`:"",p=t.metricValue?`
            ...
            <div class="post-cover">
                ${s?`<img src="${this.escapeHtml(s)}" alt="Cover" loading="lazy" decoding="async" onerror="this.style.display='none';">`:'<div class="fallback-cover-logo"><span>J</span><span>home</span></div>'}
            </div>
