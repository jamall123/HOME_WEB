import{t as c}from"./preload-helper-BATLnrmA.js";var d=class{constructor(){this.db=firebase.firestore(),this.storage=firebase.storage()}async updateProfile(t,e){if(!t)throw new Error("No UID provided");await this.db.collection("users").doc(t).set(e,{merge:!0})}uploadMedia(t,e,s=null){return new Promise((n,r)=>{const i=this.storage.ref().child(`${e}/${Date.now()}_${t.name}`).put(t);i.on("state_changed",a=>{s&&s(a.bytesTransferred/a.totalBytes*100)},a=>r(a),async()=>{n(await i.snapshot.ref.getDownloadURL())})})}async updateTeachingMode(t,e){const s={...e,updatedAt:firebase.firestore.FieldValue.serverTimestamp()};await this.db.collection("active_sessions").doc(t).set(s,{merge:!0})}async updateClassroomState(t,e){await this.db.collection("active_sessions").doc(t).set({permissions:e,updatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:!0})}async getResources(t){return(await this.db.collection("lessonResources").where("courseId","==",t).get()).docs.map(e=>({id:e.id,...e.data()}))}async postAnnouncement(t,e){await(await c(async()=>{const{commandBus:s}=await import("./CommandBus-DFXkkVZW.js");return{commandBus:s}},[])).commandBus.dispatch({domain:"generic",action:"add",payload:{collection:"lessonAnnouncements",data:{courseId:t,...e,timestamp:firebase.firestore.FieldValue.serverTimestamp()}}})}},o=new d,u=class{init(t){this.controller=t,this.cacheDOM(),this.renderDashboardLayout(),this.attachListeners()}cacheDOM(){this.mountPoint=document.getElementById("instructor-dashboard-mount"),this.tabBtn=document.getElementById("tab-btn-instructor")}renderDashboardLayout(){this.mountPoint&&(this.tabBtn&&(this.tabBtn.style.display="block"),this.mountPoint.innerHTML=`
        ...
        `)}attachListeners(){if(!this.mountPoint)return;window.InstructorAPI={setMode:e=>this.controller.setTeachingMode(e)};const t=this.mountPoint.querySelectorAll(".inst-nav");t.forEach(e=>{e.addEventListener("click",s=>{const n=s.currentTarget.getAttribute("data-view");t.forEach(i=>{i.classList.remove("btn-primary"),i.classList.add("btn-dark")}),s.currentTarget.classList.remove("btn-dark"),s.currentTarget.classList.add("btn-primary"),this.mountPoint.querySelectorAll(".inst-view").forEach(i=>i.style.display="none");const r=this.mountPoint.querySelector(`#inst-view-${n}`);r&&(r.style.display="block")})})}},b=new u,p=class{init(t){this.controller=t}async setMode(t,e={}){const s={mode:t,metadata:e};await o.updateTeachingMode(this.controller.engine.courseId,s)}},l=new p,h=class{constructor(){this.controller=null}init(t){this.controller=t,this.attachListeners()}attachListeners(){const t=document.getElementById("inst-toggle-chat"),e=document.getElementById("inst-toggle-resources");t&&t.addEventListener("change",async s=>{const n=s.target.checked;await o.updateClassroomState(this.controller.engine.courseId,{chatLocked:n})}),e&&e.addEventListener("change",async s=>{const n=s.target.checked;await o.updateClassroomState(this.controller.engine.courseId,{resourcesLocked:n})})}},m=new h,f=class{constructor(){this.controller=null,this.unsubscribe=null}init(t){this.controller=t,this.startListening()}startListening(){const t=firebase.firestore();this.unsubscribe=t.collection("courses").doc(this.controller.engine.courseId).collection("connected_users").onSnapshot(e=>{const s=document.getElementById("instructor-student-list");if(!s)return;if(e.empty){s.innerHTML='<tr><td colspan="3" style="text-align: center; padding: 1rem;">لا يوجد طلاب متصلين حالياً.</td></tr>';return}const n=document.createDocumentFragment();e.forEach(r=>{const i=r.data(),a=document.createElement("tr");a.style.borderBottom="1px solid rgba(255,255,255,0.05)",a.innerHTML=`
                    ...
                    `,n.appendChild(a)}),s.innerHTML="",s.appendChild(n)})}destroy(){this.unsubscribe&&this.unsubscribe()}},g=new f,y=class{init(t){this.controller=t}},v=new y,w=class{constructor(){this.controller=null,this.unsubscribePresence=null,this.onlineCount=0}init(t){this.controller=t,this.startListening()}startListening(){const t=firebase.firestore(),e=this.controller.engine.courseId;this.unsubscribePresence=t.collection("courses").doc(e).collection("connected_users").onSnapshot(s=>{this.onlineCount=s.size,this.updateUI()})}updateUI(){const t=document.getElementById("inst-stat-online");t&&(t.innerText=this.onlineCount)}destroy(){this.unsubscribePresence&&this.unsubscribePresence()}},I=new w,x=class{constructor(){this.engine=null,this.isInitialized=!1}init(t){this.isInitialized||(this.engine=t,this.engine.isInstructor&&(b.init(this),l.init(this),m.init(this),g.init(this),v.init(this),I.init(this),this.isInitialized=!0))}async updateProfile(t){try{await o.updateProfile(this.engine.currentUser.uid,t)}catch(e){throw e}}async setTeachingMode(t,e={}){await l.setMode(t,e)}},M=new x;export{M as InstructorController};
