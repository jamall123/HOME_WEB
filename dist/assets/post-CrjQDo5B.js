import"./ThemeManager-DVdnSx32.js";import{t as s}from"./GlobalController-BgGYEAUO.js";/* empty css             */var c=class{constructor(){this.db=window.firebase?window.firebase.firestore():null}async getPostBySlug(e){if(!this.db)return s.error("PostService: Firebase not initialized."),null;try{const t=await this.db.collection("posts").where("slug","==",e).where("status","==","published").limit(1).get();if(t.empty)return null;const a={id:t.docs[0].id,...t.docs[0].data()},i=`jhome_viewed_post_${a.id}`;return sessionStorage.getItem(i)||(sessionStorage.setItem(i,"1"),this.db.collection("posts").doc(a.id).update({views:window.firebase.firestore.FieldValue.increment(1)}).catch(n=>s.warn("Failed to increment views",n))),a}catch(t){throw s.error("PostService: getPostBySlug error:",t),t}}async getRelatedPosts(e,t){if(!this.db||!e)return[];try{return(await this.db.collection("posts").where("status","==","published").where("category","==",e).orderBy("publishedAt","desc").limit(4).get()).docs.map(a=>({id:a.id,...a.data()})).filter(a=>a.id!==t).slice(0,3)}catch(a){return s.warn("PostService: getRelatedPosts failed",a),[]}}},o=new c,l=class{constructor(){this.container=document.getElementById("postContent"),this.relatedSection=document.getElementById("relatedSection"),this.relatedGrid=document.getElementById("relatedGrid")}escapeHtml(e){if(!e)return"";const t=document.createElement("div");return t.textContent=e,t.innerHTML}renderError(e){this.container&&(this.container.innerHTML=`<div class="error-state">${this.escapeHtml(e)}</div>`)}updateSEO(e){document.getElementById("pageTitle").textContent=(e.seoTitle||e.title)+" | Jhome",document.getElementById("pageDescription").setAttribute("content",e.seoDescription||e.excerpt||""),document.getElementById("ogTitle").setAttribute("content",e.seoTitle||e.title),document.getElementById("ogDescription").setAttribute("content",e.seoDescription||e.excerpt||""),e.coverImage&&document.getElementById("ogImage").setAttribute("content",e.coverImage)}renderPost(e){if(!this.container)return;this.updateSEO(e);const t=e.publishedAt?new Date(e.publishedAt.seconds*1e3).toLocaleDateString("ar-SD",{year:"numeric",month:"long",day:"numeric"}):"",a=e.tags&&e.tags.length?`
        ...
        `).join(""))}},r=new l,d=class{async init(){const e=new URLSearchParams(window.location.search).get("slug");if(!e){r.renderError("لم يتم تحديد مقال.");return}try{const t=await o.getPostBySlug(e);if(!t){r.renderError("عذراً، المقال غير موجود.");return}r.renderPost(t),o.getRelatedPosts(t.category,t.id).then(a=>r.renderRelated(a)).catch(a=>s.warn("PostController: Failed to load related",a))}catch(t){s.error("PostController: Failed to load post",t),r.renderError("حدث خطأ في تحميل المقال.")}}},h=new d;document.addEventListener("DOMContentLoaded",()=>{h.init()});
