import"./firebase-config-teItJWtm.js";import"./GlobalController-DC8vr5AO.js";import{t as s}from"./Logger-D7Tvbo4W.js";/* empty css             */var o=class{constructor(){this.db=window.firebase?window.firebase.firestore():null}async getPostBySlug(e){if(!this.db)return s.error("PostService: Firebase not initialized."),null;try{const t=await this.db.collection("posts").where("slug","==",e).where("status","==","published").limit(1).get();if(t.empty)return null;const a={id:t.docs[0].id,...t.docs[0].data()};return this.db.collection("posts").doc(a.id).update({views:window.firebase.firestore.FieldValue.increment(1)}).catch(i=>s.warn("Failed to increment views",i)),a}catch(t){throw s.error("PostService: getPostBySlug error:",t),t}}async getRelatedPosts(e,t){if(!this.db||!e)return[];try{return(await this.db.collection("posts").where("status","==","published").where("category","==",e).orderBy("publishedAt","desc").limit(4).get()).docs.map(a=>({id:a.id,...a.data()})).filter(a=>a.id!==t).slice(0,3)}catch(a){return s.warn("PostService: getRelatedPosts failed",a),[]}}},n=new o,l=class{constructor(){this.container=document.getElementById("postContent"),this.relatedSection=document.getElementById("relatedSection"),this.relatedGrid=document.getElementById("relatedGrid")}escapeHtml(e){if(!e)return"";const t=document.createElement("div");return t.textContent=e,t.innerHTML}renderError(e){this.container&&(this.container.innerHTML=`<div class="error-state">${this.escapeHtml(e)}</div>`)}updateSEO(e){document.getElementById("pageTitle").textContent=(e.seoTitle||e.title)+" | Jhome",document.getElementById("pageDescription").setAttribute("content",e.seoDescription||e.excerpt||""),document.getElementById("ogTitle").setAttribute("content",e.seoTitle||e.title),document.getElementById("ogDescription").setAttribute("content",e.seoDescription||e.excerpt||""),e.coverImage&&document.getElementById("ogImage").setAttribute("content",e.coverImage)}renderPost(e){if(!this.container)return;this.updateSEO(e);const t=e.publishedAt?new Date(e.publishedAt.seconds*1e3).toLocaleDateString("ar-SD",{year:"numeric",month:"long",day:"numeric"}):"",a=e.tags&&e.tags.length?`
          <div class="post-tags">
            ${e.tags.map(i=>`<span class="tag">#${this.escapeHtml(i)}</span>`).join("")}
          </div>
        `:"";this.container.innerHTML=`
            <div class="post-cover">
                <img src="${e.coverImage||"assets/images/blog-placeholder.jpg"}" alt="${this.escapeHtml(e.title)}">
            </div>
        ...
        `).join(""))}},r=new l,c=class{async init(){const e=new URLSearchParams(window.location.search).get("slug");if(!e){r.renderError("لم يتم تحديد مقال.");return}try{const t=await n.getPostBySlug(e);if(!t){r.renderError("عذراً، المقال غير موجود.");return}r.renderPost(t),n.getRelatedPosts(t.category,t.id).then(a=>r.renderRelated(a)).catch(a=>s.warn("PostController: Failed to load related",a))}catch(t){s.error("PostController: Failed to load post",t),r.renderError("حدث خطأ في تحميل المقال.")}}},d=new c;document.addEventListener("DOMContentLoaded",()=>{d.init()});
