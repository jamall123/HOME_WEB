...
export const contact = functions.https.onCall(async (rawData, context) => {
  ...
  try {
    switch (action) {
      ...
      // ── MARK READ ────────────────────────────────────────────────────────
      case 'markRead': {
        if (!isAdmin) throw new functions.https.HttpsError('permission-denied', 'Only admins can mark messages.');
      ...
      // ── REPLY ────────────────────────────────────────────────────────────
      case 'reply': {
        if (!isAdmin) throw new functions.https.HttpsError('permission-denied', 'Only admins can reply to messages.');
        if (!id || !replyMessage) throw new functions.https.HttpsError('invalid-argument', 'Missing id or replyMessage.');
      ...
      // ── DELETE ───────────────────────────────────────────────────────────
      case 'delete': {
        if (!isAdmin) throw new functions.https.HttpsError('permission-denied', 'Only admins can delete messages.');
        if (!id) throw new functions.https.HttpsError('invalid-argument', 'Missing id.');
