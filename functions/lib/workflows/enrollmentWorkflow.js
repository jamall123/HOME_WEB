...
import { EventType } from '../events/eventBus.js';
export class EnrollmentWorkflow {
    async execute(requestId, requestData) {
        ...
        try {
            ...
            // 3. Send Email
            await notificationService.sendEmail(requestData.email, 'Welcome to your new Jhome Course!', `Hello ${requestData.name}, your enrollment in course ${requestData.courseId} has been approved.`);
            // 4. Send Notification
            await notificationService.sendSystemNotification(requestData.userId, 'Enrollment Approved', `You have been granted access to course ${requestData.courseId}.`);
            // 5. Emit Event
            await DI.eventBus.publish({
                type: EventType.USER_APPROVED,
                payload: { userId: requestData.userId, courseId: requestData.courseId, requestId },
