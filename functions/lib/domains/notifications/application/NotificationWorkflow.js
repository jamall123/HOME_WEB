...
export class NotificationWorkflow {
    ...
    registerListeners() {
        DI.eventBus.subscribe(EventType.USER_APPROVED, async (payload) => {
            DI.logger.info(`NotificationWorkflow received USER_APPROVED event for user ${payload.userId}`);
            try {
                await notificationService.sendEmail(payload.email, 'Welcome to your new Jhome Course!', `Hello ${payload.name}, your enrollment in course ${payload.courseId} has been approved.`);
                await notificationService.sendSystemNotification(payload.userId, 'Enrollment Approved', `You have been granted access to course ${payload.courseId}.`);
            }
            catch (error) {
...
//# sourceMappingURL=NotificationWorkflow.js.map