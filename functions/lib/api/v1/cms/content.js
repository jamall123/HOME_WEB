...
const ALLOWED_ENTITIES = ['post', 'story', 'project', 'page'];
...
const WRITE_ROLES = [Role.SUPER_ADMIN, Role.ADMIN, Role.EDITOR, Role.AUTHOR];
export const content = functions.https.onCall(async (rawData, context) => {
    ...
    if (!entity || !ALLOWED_ENTITIES.includes(entity)) {
        throw new functions.https.HttpsError('invalid-argument', `Invalid entity. Must be one of: ${ALLOWED_ENTITIES.join(', ')}.`);
    }
    ...
    try {
        switch (action) {
            ...
            case 'save': {
                ...
                if (!contentData)
                    throw new functions.https.HttpsError('invalid-argument', 'Missing data in payload.');
                const docRef = id
